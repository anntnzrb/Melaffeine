...
NSNotificationName const PowerControllerDidChangeNotification =
    @"PowerControllerDidChangeNotification";
...
- (BOOL)startWithDuration:(NSNumber *)durationSeconds
         keepDisplayAwake:(BOOL)keepDisplayAwake
                    error:(NSError **)error {
  [self stop];

  CFStringRef assertionType =
      keepDisplayAwake ? kIOPMAssertionTypePreventUserIdleDisplaySleep
                       : kIOPMAssertionTypePreventUserIdleSystemSleep;

  IOPMAssertionID newAssertionID = kIOPMNullAssertionID;
  IOReturn result = IOPMAssertionCreateWithName(
      assertionType, kIOPMAssertionLevelOn, (__bridge CFStringRef)CTAppName,
      &newAssertionID);

  if (result != kIOReturnSuccess) {
    if (error) {
      *error = [NSError errorWithDomain:CTPowerErrorDomain
                                   code:result
                               userInfo:nil];
    }
    return NO;
  ...
  self.startedAt = now;
  self.endsAt = durationSeconds
                    ? [now dateByAddingTimeInterval:durationSeconds.doubleValue]
                    : nil;
  self.keepDisplayAwake = keepDisplayAwake;
  self.active = YES;
  [NSNotificationCenter.defaultCenter
      postNotificationName:PowerControllerDidChangeNotification
                    object:self];

  if (durationSeconds) {
    self.timer =
        [NSTimer scheduledTimerWithTimeInterval:durationSeconds.doubleValue
                                         target:self
