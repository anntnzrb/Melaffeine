...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
  ...
  [NSNotificationCenter.defaultCenter
      addObserver:self
         selector:@selector(powerDidChange:)
             name:PowerControllerDidChangeNotification
           object:self.power];
}
...
- (NSView *)buildContentView {
  NSView *view = [[NSView alloc]
      initWithFrame:NSMakeRect(0, 0, CTMenuWidth, CTMenuHeight)];
  CGFloat x = CTMenuPadding;
  CGFloat width = CTMenuWidth - (CTMenuPadding * 2.0);

  self.indefiniteButton =
      [NSButton checkboxWithTitle:CTTitleRunIndefinitely
                           target:self
                           action:@selector(controlChanged:)];
  self.indefiniteButton.frame =
      NSMakeRect(x, CTIndefiniteY, width, CTCheckboxHeight);
  ...
  self.durationFormatter.maximum =
      @(CTMaximumFiniteDurationSeconds / CTSecondsPerMinute);
  self.durationField.formatter = self.durationFormatter;
  self.durationField.frame =
      NSMakeRect(x, CTDurationY, CTDurationFieldWidth, CTTextFieldHeight);

  [view addSubview:self.durationField];

  self.unitPopup = [NSPopUpButton new];
  self.unitPopup.controlSize = NSControlSizeRegular;
  [self.unitPopup addItemsWithTitles:@[
    CTUnitMinutesTitle, CTUnitHoursTitle, CTUnitDaysTitle
  ]];
  [self.unitPopup selectItemAtIndex:CTUnitHoursIndex];
  self.unitPopup.frame =
      NSMakeRect(x + CTDurationFieldWidth + CTControlSpacing, CTUnitPopupY,
                 CTUnitPopupWidth, CTPopupHeight);

  [view addSubview:self.unitPopup];

  self.keepDisplayAwakeButton =
      [NSButton checkboxWithTitle:CTTitleKeepDisplayAwake
                           target:nil
                           action:nil];
  self.keepDisplayAwakeButton.frame =
      NSMakeRect(x, CTDisplayAwakeY, width, CTCheckboxHeight);
  ...
  self.startStopButton =
      [NSButton buttonWithTitle:CTTitleStart
                         target:self
                         action:@selector(startStopClicked:)];
  self.startStopButton.bezelStyle = NSBezelStyleRounded;
  self.startStopButton.frame =
      NSMakeRect(x, CTStartButtonY, CTStartButtonWidth, CTButtonHeight);

  [view addSubview:self.startStopButton];

  self.launchAtLoginButton =
      [NSButton checkboxWithTitle:CTTitleLaunchAtLogin
                           target:self
                           action:@selector(launchAtLoginChanged:)];
  self.launchAtLoginButton.frame =
      NSMakeRect(x + CTLaunchAtLoginXOffset, CTLaunchAtLoginY,
                 CTLaunchAtLoginWidth, CTCheckboxHeight);
...
- (void)showContextMenu {
  ...
  [menu addItem:[[NSMenuItem alloc] initWithTitle:CTTitleQuit
...
- (void)startStopClicked:(id)sender {
  ...
  BOOL ok = [self.power startWithDuration:duration
                         keepDisplayAwake:self.keepDisplayAwakeButton.state ==
...
- (NSNumber *)selectedDuration {
  if (self.indefiniteButton.state == NSControlStateValueOn) {
    return nil;
  }
...
- (void)updateStateUI {
  ...
  self.durationField.enabled =
      !active && self.indefiniteButton.state != NSControlStateValueOn;
  self.unitPopup.enabled =
      !active && self.indefiniteButton.state != NSControlStateValueOn;
...
- (void)updateStatusIconActive:(BOOL)active {
  NSImage *image = [NSImage
      imageWithSystemSymbolName:(active ? CTIconActive : CTIconInactive)
       accessibilityDescription:CTAppName];
  image.template = YES;
...
- (void)updateCountdownUI {
  ...
  self.timeLabel.stringValue =
      [NSString stringWithFormat:CTCountdownFormat, CTCountdownStopsInPrefix,
                                 duration, CTCountdownAtSeparator, time];
  self.timeLabel.hidden = NO;
}

- (void)startCountdownTimerIfNeeded {
  if (self.countdownTimer || !self.popover.shown || !self.power.active ||
      !self.power.endsAt) {
    return;
  }
  ...
  __weak typeof(self) weakSelf = self;
  self.countdownTimer =
      [NSTimer scheduledTimerWithTimeInterval:CTCountdownUpdateInterval
                                      repeats:YES
                                        block:^(NSTimer *timer) {
...
- (void)syncLaunchAtLoginState {
  if (@available(macOS 13.0, *)) {
    self.launchAtLoginButton.state =
